var data = [
  ...
  function getValuesByAgreement(region, degree) {
      return data.filter(item => item.Territory === region && item["Degree of agreement"] === degree);
  }
  ...
  var regionsOpinion = [...new Set(data.map(item => item.Territory))];
  var stereotypesOpinion = [...new Set(data.map(item => item["Stereotypes about gender roles"]))];
