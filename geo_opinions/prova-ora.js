...
function updateChart() {
    ...
    var trace = selectedTerritories.map(territory => {
        var stronglyAgree = getValuesByAgreement(selectedStereotype, "strongly agree")
            .filter(item => item.Territory === territory)
            .map(item => item["Stereotypes values"])[0] || 0;
        var somewhatAgree = getValuesByAgreement(selectedStereotype, "somewhat agree")
            .filter(item => item.Territory === territory)
            .map(item => item["Stereotypes values"])[0] || 0;
        var somewhatDisagree = getValuesByAgreement(selectedStereotype, "somewhat disagree")
            .filter(item => item.Territory === territory)
            .map(item => item["Stereotypes values"])[0] || 0;
        var stronglyDisagree = getValuesByAgreement(selectedStereotype, "strongly disagree")
            .filter(item => item.Territory === territory)
            .map(item => item["Stereotypes values"])[0] || 0;
    ...
    // Aggiungiamo solo i marker neri per le regioni nella legenda
    ...
    var layout = {
        ...
        yaxis: {
            title: 'Stereotypes values'
        },
        showlegend: true,
        legend: {
            x: 1,     // Sposta la legenda più a sinistra rispetto a 1.05
        ...
        width: 900,                // Larghezza fissa del grafico
        height: 500,               // Altezza fissa del grafico
        margin: {
            l: 50,                  // Spazio a sinistra
            r: 100,                 // Spazio a destra per la legenda
            t: 20,                  // Spazio sopra
            b: 50                   // Spazio sotto
        }
    };
